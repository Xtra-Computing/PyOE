...
def compute_loss(net, window_x, window_y, task, tree=False, y_outlier=None):
    if task == "classification":
        ...
        if y_outlier is None:
            ...
            acc = (pred_label == window_y)*(1 - y_outlier).sum().item() / (window_y.shape[0] - y_outlier.sum().item())
        return 1-acc
    else:
        if tree:
        ...
        if y_outlier is None:
            ...
            loss = torch.sum(torch.square(out - window_y)*(1 - y_outlier)) / (window_y.shape[0] - y_outlier.sum().item())
        return loss.item()


class BasicTrainer:
    def __init__(self, dataloader, basic_model, algorithm="naive", device="cpu", lr=0.01, epochs=1, batch_size=64, buffer_size=100):
        ...
        if self.algorithm == "naive":
            if basic_model.get_model_type() not in ("tree", "tabnet", "gbdt"):
                self.optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, net.parameters()), lr=lr)
            if dataloader.task == "classification":
                self.criterion = nn.CrossEntropyLoss().to(device)
            elif dataloader.task == "regression":
                self.criterion = nn.MSELoss().to(device)
            else:
                raise ValueError("Task not supported. BasicTrainer only supports classification and regression.")
    ...
    def train(self, X, y, y_outlier, need_test=False):
        start_time = time.time()
        X, y, y_outlier = torch.Tensor(X).to(self.device), torch.Tensor(y).to(self.device), torch.Tensor(y_outlier).to(self.device)
        task = dataloader.task
        model = basic_model.get_model_type()
        net = basic_model.get_net().to(self.device)
        ...
        if need_test:
            if model == "armnet":
                ...
                accuracy_loss = compute_loss(net, x_tmp, y, task, tree=(model in ("tree", "tabnet", "gbdt")), y_outlier=y_outlier)
            else:
                accuracy_loss = compute_loss(net, X, y, task, tree=(model in ("tree", "tabnet", "gbdt")), y_outlier=y_outlier)
                ...
                if model in ("mlp", "armnet"):
                    for batch_ind in range(0,length,self.batch_size):
                        ...
                        if model == "armnet":
                            x_tmp = dict({})
                            x_tmp['value'] = batch_x
                            x_tmp['id'] = id.repeat((batch_x.shape[0],1)).to(self.device)
                            batch_x = x_tmp
                    ...
                    net.fit(window_x, window_y, batch_size=self.batch_size, virtual_batch_size=self.batch_size, max_epochs=self.epochs)
                    break
                else:
                    net.fit(X, y)
                ...
                if model in ("mlp"):
                    for batch_ind in range(0,length,self.batch_size):
                        ...
                        if self.x_example is not None:
                            ...
                            loss += criterion(out_e, self.y_example)
            ...
            if task == "regression":
                ...
                for i in range(self.dataloader.get_output_dim()):
                    if len(self.examples[i]) < buffer_class and i in y:
                        ...
                        avg = torch.mean(features, dim=0).unsqueeze(0)
                        distance = torch.norm(features-avg,dim=1)
                        _, indices = torch.topk(distance, k=min(buffer_class-len(self.examples[i]),len(distance)), largest=False)
                        if len(self.examples[i])==0:
                            self.examples[i] = data_class[indices]
                        else:
                            self.examples[i] = torch.cat((self.examples[i],data_class[indices]),dim=0)
                        update = True
                if update:
                    self.x_example = []
                    for i in range(self.dataloader.get_output_dim()):
                        ...
                        if len(self.x_example)==0:
                            ...
                            self.x_example = torch.cat((self.x_example, self.examples[i]),dim=0)
                            self.y_example = torch.cat((self.y_example,torch.ones(self.examples[i].shape[0])*i),dim=0)
                    if task == "classification":
                        self.y_example = self.y_example.long()
                    self.x_example, self.y_example = self.x_example.to(self.device), self.y_example.to(self.device)
